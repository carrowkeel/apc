...
export const initializePlots = (container, plots) => {
	plots.forEach(plot => {
		...
		plot_elem.innerHTML = `<div class="crosshair"></div><div class="header"><a data-icon="f" data-action="export" class="settings fright"></a><div class="title">${plot.label}</div></div><div class="legend">${legend}</div><div class="draw_area"><div class="axis-label label-x">${mathNotation(plot.xlabel)}</div><div class="axis-label label-y">${mathNotation(plot.ylabel)}</div><div class="axis-tick xmin">${shortNotation(plot.xlim[0])}</div><div class="axis-tick ymin editable" title="Edit y-axis">${shortNotation(plot.ylim[0])}</div><div class="axis-tick xmax">${shortNotation(plot.xlim[1])}</div><div class="axis-tick ymax editable" title="Edit y-axis">${shortNotation(plot.ylim[1])}</div></div></div>`;
	...
	plots.forEach(plot => {
		...
		switch(true) {
			...
			case plot.data !== undefined:
				const plot_data = plot.data.split(',').map(prop => data[data.length - 1][prop]);
	...
	mat: (draw, data, i) => {
		...
		data.forEach((row, y) => {
			row.forEach((cell, x) => {
				...
				draw.square([x / row.length, y / data.length, 1 / row.length, 1 / data.length], color);
			});
		});
		draw.post();
	},
...
export const canvas_draw = (canvas, ctx, bounds=[[0, 1], [0, 1]], dims=[canvas.dataset.width, canvas.dataset.height].map(v=>+(v)), scale=bounds.map((v,i)=>dims[i]/(v[1]-v[0]))) => ({
	...
	square: function (s, color, opacity=1, stroke=true) {
		const [x, y] = [scale[0] * (s[0] - bounds[0][0]), scale[1] * (bounds[1][1]-s[1]-s[3])];
		const rect = [
			x,
			y,
