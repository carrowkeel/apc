...
export const initializePlots = (container, plots) => {
	plots.forEach(plot => {
		...
		plot_elem.innerHTML = `<div class="crosshair"></div><div class="header"><a data-icon="e" data-action="export" class="settings fright"></a><div class="title">${plot.label}</div></div><div class="legend">${legend}</div><div class="draw_area"><div class="axis-label label-x">${mathNotation(plot.xlabel)}</div><div class="axis-label label-y">${mathNotation(plot.ylabel)}</div><div class="axis-tick xmin">${shortNotation(plot.xlim[0])}</div><div class="axis-tick ymin editable" title="Edit y-axis">${shortNotation(plot.ylim[0])}</div><div class="axis-tick xmax">${shortNotation(plot.xlim[1])}</div><div class="axis-tick ymax editable" title="Edit y-axis">${shortNotation(plot.ylim[1])}</div></div></div>`;
